...
`COUNT(*)`和`COUNT(id)`在这里是一样的效果。另外注意，聚合查询同样可以使用`WHERE`条件，因此我们可以方便地统计出有多少男生、多少女生、多少 80 分以上的学生等：
...
1. count(表达式) --分组里非空记录数
2. count(\*) -- 所有记录
3. count(1) --所有记录
4. count(case job = 'SALESMAN' then 2 end )-- SALESMAN 人数
5. count(comm)--comm 字段不为 NULL 的记录数
6. count(distinct job)--distinct(去重)，共有多少种工作
